...
//This callback is for the appsink, rather than the message bus
...
JNIEXPORT jlong JNICALL Java_processing_streaming_Streaming_gstreamer_1loadFile(JNIEnv *env, jobject obj, jstring _fn)
{
    GError *error = NULL;
    ...
    switch (ret) {
      ...
      case GST_STATE_CHANGE_NO_PREROLL:
        ...
        //exit (-1);
      	//Oh wait, but we are gonna do it now
      	//ret = gst_element_set_state (v->play, GST_STATE_PLAYING);
      	live = TRUE;
      	g_print("just set live to true\n");
      	/*
      	if(ret == GST_STATE_CHANGE_FAILURE){
        	g_print ("Can't just play this file\n");
        	exit (-1);
        }
        */
      default:
      	g_print("Other status message after set to playing");
        break;
    }
    ...
    /* get the duration */
    gint64 duration;
    //if the query is not true, that means that we don't know the duration (most likely live source)
    //this means we need to be careful about null references to duration
    gboolean query = gst_element_query_duration (v->play, GST_FORMAT_TIME, &duration);
    if(query == FALSE)
    {
    	g_print("Woah there, no stable duration. Might be a live stream, and might just die\n");
    }  
